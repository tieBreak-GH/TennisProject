...
## 8. Ral(l)i Segmentasyonu ve Servis Tespiti

Kullanıcıyla teknik/mantık/UX-UI eksenlerinde bir beyin fırtınası yapıldı; kullanıcı "vuruş sınıflandırması"nı seçti. Forehand/backhand ayrımının poz (pose) tahmini + oyuncu el tercihi bilgisi gerektirdiği (yeni model, belirsiz doğruluk) kullanıcıya açıkça anlatıldı; kullanıcı bunun yerine **sadece servis tespitini** (yeni model yok, net geometrik kural) seçti.

- **`rally_analyzer.py`** (yeni dosya): `segment_rallies` top takibindeki >1.5s'lik kesintileri rali sınırı sayar; `segment_shots` (mevcut `get_shot_max_speed` desenini tekrar kullanarak) her raliyi sekme sınırlarıyla vuruşlara böler; `analyze_rallies` her ralinin ilk vuruşunu, başlangıç noktası homografiyle court-plane'e projelendirilip bir baseline'a (`CourtReference.baseline_top`/`baseline_bottom`) yakın çıkarsa servis olarak etiketler.
- **Bilinen kısıt:** `max_gap_seconds=1.5` ve `baseline_margin_cm=300` gerçek veriyle kalibre edilmiş sabitler değil, makul varsayılan tahminler (YOLO eşiği örneğinde olduğu gibi gerçek görüntüyle ayarlanması gerekebilir). Klip rali ortasından başlıyorsa ilk vuruş "gap sonrası" sınırına girer ama baseline yakınlığı kontrolü çoğu durumda bunu doğru şekilde servis-olmayan olarak eler.
- **`main.py`:** `process_video` içinde `analyze_rallies` çağrılıp `stats['rallies']`/`stats['num_rallies']`'e ekleniyor; `main()` render fonksiyonu artık `serve_frames` parametresi alıp topun yanına (mevcut hız etiketinin altına) sarı `"SERVIS"` etiketi çiziyor.
- **`app.py`:** Sonuç ekranına, her rali için vuruş sayısı/süre/servis var mı/ort.-maks. hız gösteren bir tablo eklendi.
- **Doğrulama:** `rally_analyzer.py` sentetik testlerle (rali/vuruş sınırları, baseline yakınlık kontrolü, servis pozitif/negatif senaryoları) doğrulandı. Gerçek ATP maçı klibiyle uçtan uca çalıştırıldığında (klip rali ortasından başlıyor) beklendiği gibi **0 servis tespit edildi** — ilk vuruşun court-plane konumu (y≈2286, baseline'lardan >600 birim uzakta) doğru şekilde reddedildi, bu da baseline-yakınlık filtresinin klip-ortası yanlış pozitiflerine karşı çalıştığını doğruluyor. "SERVIS" etiketinin çizim mantığı ayrı bir sentetik render testiyle görsel olarak da doğrulandı.
- **Henüz yapılmadı:** Gerçekten bir servisle başlayan bir klip üzerinde pozitif doğrulama (elimde böyle bir test videosu yok) — mevcut testler negatif senaryoyu (yanlış pozitif üretmeme) doğruluyor, gerçek bir serviste doğru tespit ettiğini henüz canlı görmedik.
